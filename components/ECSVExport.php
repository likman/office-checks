...
class ECSVExport
{
    ...
    public function toCSV($outputFile=null, $delimiter=null, $enclosure=null, $includeHeaders=true)
    {
        ...
        if($this->_dataProvider instanceof \yii\data\ActiveDataProvider) {
            $isGood = true;
        }
        
        if($this->_dataProvider instanceof \yii\data\SqlDataProvider) {
            $isGood = true;
        }
    ...
    {
        $firstTimeThrough = true;
        if($dp instanceof \yii\db\DataReader) {
    ...
    protected function _writeHeaders($row)
    {
        ...
        if($row instanceof \yii\db\ActiveRecord) {
    ...
    public function _writeRow($row)
    {
        ...
        if($this->stripNewLines) {            
            array_walk($row, ['\app\components\ECSVExport','lambdaFail']);
        }
        
        array_walk($row, ['\app\components\ECSVExport','stripSlashes']);
